...
namespace Autocad_Primavera_P6_Plugin.Services.AutocadService.Temp
{
    public static class PlugInBlockReference_Helpers
    {
        public static Dictionary<string, AttributeReference> Get_AttRefDict(BlockReference acadBlockRef, Transaction tr)
        {
            return acadBlockRef.AttributeCollection
                ...
                .ToDictionary(ar => ar.Tag, ar => ar, StringComparer.OrdinalIgnoreCase);
        }
        ...
        public static Dictionary<string, DynamicBlockReferenceProperty> Get_DyBlockRefPropDict(BlockReference acadBlockRef)
        {
            return acadBlockRef.DynamicBlockReferencePropertyCollection
                ...
                .ToDictionary(p => p.PropertyName, p => p, StringComparer.OrdinalIgnoreCase);
        }
        ...
        {
            ["Slot_1"] = "BOUNDARY_CODE_ID", ["Slot_2"] = "BOUNDARY_CODE_PATH", ["Slot_3"] = "BOUNDARY_CODE_VALUE",
            ["Slot_4"] = "ELEMENT_ID_CODE_ID", ["Slot_5"] = "ELEMENT_ID_CODE_PATH", ["Slot_6"] = "ELEMENT_ID_CODE_VALUE",
            ["Slot_7"] = "LENGTH_L", ["Slot_8"] = "BREADTH_B", ["Slot_9"] = "HEIGHT_H"
        };
    }
    ...
    public class Slot
    {
        ...
        public static Slot New_Slot(Dictionary<string, AttributeReference> attRefDict, int slotIndex)
        {
            ...
            return new Slot { SlotIndex = slotIndex, SlotName = slotName, NameAttRefTag = nameAttRefTag, ValueAttRefTag = valueAttRefTag, NameAttRef = nameAttribute, ValueAttRef = valueAttribute };
        }
    }
    ...
    public class PlugInBlockReference
    {
        ...
        public PlugInBlockReference(BlockReference blockRef, bool skipInit = false) { AcadBlockRef = blockRef; if (!skipInit) { _ = Async_Init(); } }
        ...
        public async Task Async_Init(Transaction tr = null)
        {
            ...
            PlugInBlockReference_Helpers.ExecuteWithTransaction(AcadDoc.Database, tr, currTr =>
            {
                ...
                foreach (var key in SlotsDict.Keys.ToList())
                {
                    ...
                    if (_newSlot != null && !_newSlot.IsNoProperty) { PropSlotDict[_newSlot.NameAttRefTag] = _newSlot.SlotName; }
                }
        ...
        private Slot Get_PropertySlot(string propertyName) { var slotFound = PropSlotDict.TryGetValue(propertyName, out string slotName); if (!slotFound) { return null; } return SlotsDict[slotName]; }
        public string Get_PropertySlotValue(string propertyName) { var propertySlot = Get_PropertySlot(propertyName); if (propertySlot == null || propertySlot.IsNoValue) { return null; } return propertySlot.ValueAttRef.TextString; }

        public bool Set_PropertySlotValue(string propertyName, string updateValue, Transaction tr, out string result)
        {
            TransactionResult<(bool Success, string OutValue)> operationResult = PlugInBlockReference_Helpers.ExecuteWithDocumentLock(AcadDoc, () => PlugInBlockReference_Helpers.ExecuteWithTransaction(AcadDoc.Database, tr, currTr =>
            {
                ...
                string _defaultSlotName = PlugInBlockReference_Helpers.DefaultPropSlotDict.FirstOrDefault(kv => kv.Value.Equals(propertyName, StringComparison.OrdinalIgnoreCase)).Key;
                Slot _targetSlot = null;
                if (_defaultSlotName != null && SlotsDict.TryGetValue(_defaultSlotName, out Slot _defaultSlot) && _defaultSlot != null && _defaultSlot.IsNoProperty) { _targetSlot = _defaultSlot; }
                else { _targetSlot = SlotsDict.Where(kv => !PlugInBlockReference_Helpers.DefaultPropSlotDict.ContainsKey(kv.Key)).Select(kv => kv.Value).FirstOrDefault(s => s != null && s.IsNoProperty); }
                if (_targetSlot == null) { return new TransactionResult<(bool, string)>((false, null), TransactionAction.Nothing); }
                var _nameAttRef = (AttributeReference)currTr.GetObject(_targetSlot.NameAttRef.ObjectId, OpenMode.ForWrite); _nameAttRef.TextString = propertyName;
                var _valueAttRef = (AttributeReference)currTr.GetObject(_targetSlot.ValueAttRef.ObjectId, OpenMode.ForWrite); _valueAttRef.TextString = updateValue;
                PropSlotDict[propertyName] = _targetSlot.SlotName;
                return new TransactionResult<(bool, string)>((true, updateValue), TransactionAction.Commit);
            }));
            result = operationResult.Result.OutValue; return operationResult.Result.Success;
        }

        public bool Set_InfoPosition(Point3d targetPosition, out Point3d? resultPosition, Transaction tr = null)
        {
            TransactionResult<(bool Success, Point3d? OutValue)> operationResult = PlugInBlockReference_Helpers.ExecuteWithDocumentLock(AcadDoc, () => PlugInBlockReference_Helpers.ExecuteWithTransaction(AcadDoc.Database, tr, currTr =>
            {
                ...
                Matrix3d _wcsToBlock = _blockRef.BlockTransform.Inverse(); Point3d _localPoint = targetPosition.TransformBy(_wcsToBlock);
                var _dyBlockRefPropDict = PlugInBlockReference_Helpers.Get_DyBlockRefPropDict(_blockRef);
                var _dyProperty_X = PlugInBlockReference_Helpers.TryGetValue_DyBlockRefPropDict(_dyBlockRefPropDict, "MoveInfo X");
                var _dyProperty_Y = PlugInBlockReference_Helpers.TryGetValue_DyBlockRefPropDict(_dyBlockRefPropDict, "MoveInfo Y");
                if (_dyProperty_X == null || _dyProperty_Y == null) { return new TransactionResult<(bool, Point3d?)>((false, null), TransactionAction.Nothing); }
                _dyProperty_X.Value = _localPoint.X; _dyProperty_Y.Value = _localPoint.Y;
                return new TransactionResult<(bool, Point3d?)>((true, targetPosition), TransactionAction.Commit);
            }));
            resultPosition = operationResult.Result.OutValue; return operationResult.Result.Success;
        }
        ...
        public bool Set_BlockPosition(Point3d newPosition, out Point3d resultPosition, Transaction tr = null)
        {
            TransactionResult<(bool Success, Point3d OutValue)> operationResult = PlugInBlockReference_Helpers.ExecuteWithDocumentLock(AcadDoc, () => PlugInBlockReference_Helpers.ExecuteWithTransaction(AcadDoc.Database, tr, currTr =>
            {
                var _blockRef = (BlockReference)currTr.GetObject(AcadBlockRef.ObjectId, OpenMode.ForWrite); _blockRef.Position = newPosition;
        ...
        {
            var boundaryCodeIdString = Get_PropertySlotValue("BOUNDARY_CODE_ID"); if (!int.TryParse(boundaryCodeIdString, out int boundaryCodeId)) { return null; }
            var actCode = await PluginInstance.MyP6ApiService.GetP6ActivityCodeById(boundaryCodeId); return actCode;
        }

        public bool Set_BdryActCode(ActivityCode actCode, out ActivityCode result, Transaction tr = null)
        {
            TransactionResult<(bool Success, ActivityCode OutValue)> operationResult = PlugInBlockReference_Helpers.ExecuteWithDocumentLock(AcadDoc, () => PlugInBlockReference_Helpers.ExecuteWithTransaction(AcadDoc.Database, tr, currTr =>
            {
                var idResult = Set_PropertySlotValue("BOUNDARY_CODE_ID", actCode.ObjectId.ToString(), currTr, out _);
                var valueResult = Set_PropertySlotValue("BOUNDARY_CODE_VALUE", actCode.Description, currTr, out _);
                var pathResult = Set_PropertySlotValue("BOUNDARY_CODE_PATH", actCode.CodeConcatName, currTr, out _);
                return !idResult || !valueResult || !pathResult ? new TransactionResult<(bool, ActivityCode)>((false, null), TransactionAction.Abort) : new TransactionResult<(bool, ActivityCode)>((true, actCode), TransactionAction.Commit);
            }));
            result = operationResult.Result.OutValue; return operationResult.Result.Success;
        }
        ...
        {
            var elementIdCodeIdString = Get_PropertySlotValue("ELEMENT_ID_CODE_ID"); if (!int.TryParse(elementIdCodeIdString, out int elementIdCodeId)) { return null; }
            var actCode = await PluginInstance.MyP6ApiService.GetP6ActivityCodeById(elementIdCodeCodeId);
            return actCode;
        }
        ...
        public bool Set_ElementIdCode(ActivityCode actCode, out ActivityCode result, Transaction tr = null)
        {
            TransactionResult<(bool Success, ActivityCode OutValue)> operationResult = PlugInBlockReference_Helpers.ExecuteWithDocumentLock(AcadDoc, () => PlugInBlockReference_Helpers.ExecuteWithTransaction(AcadDoc.Database, tr, currTr =>
            {
                var idResult = Set_PropertySlotValue("ELEMENT_ID_CODE_ID", actCode.ObjectId.ToString(), currTr, out _);
                var valueResult = Set_PropertySlotValue("ELEMENT_ID_CODE_VALUE", actCode.Description, currTr, out _);
                var pathResult = Set_PropertySlotValue("ELEMENT_ID_CODE_PATH", actCode.CodeConcatName, currTr, out _);
                if (!idResult || !valueResult || !pathResult) { return new TransactionResult<(bool, ActivityCode)>((false, null), TransactionAction.Abort); }
                Reset_ElementId(out _, currTr);
        ...
        {
            static string Get_ElementIdString(string pathString) { try { return string.Join(" -> ", pathString.Split('.')[^2..]); } catch { return string.Empty; } }
            TransactionResult<(bool Success, string OutValue)> operationResult = PlugInBlockReference_Helpers.ExecuteWithDocumentLock(AcadDoc, () => PlugInBlockReference_Helpers.ExecuteWithTransaction(AcadDoc.Database, tr, currTr =>
            {
                var _elementIdCodePath = Get_PropertySlotValue("ELEMENT_ID_CODE_PATH"); var _elementIdString = Get_ElementIdString(_elementIdCodePath);
                var _elementId_AttRef = (AttributeReference)currTr.GetObject(ElementId_AttRef.ObjectId, OpenMode.ForWrite); _elementId_AttRef.TextString = _elementIdString;
        ...
        {
            TransactionResult<(bool Success, Group OutValue)> operationResult = PlugInBlockReference_Helpers.ExecuteWithTransaction(AcadDoc.Database, tr, currTr =>
            {
                try
                {
                    DBDictionary groupDict = (DBDictionary)currTr.GetObject(AcadDoc.Database.GroupDictionaryId, OpenMode.ForRead);
                    foreach (DBDictionaryEntry entry in groupDict) { Group grp = (Group)currTr.GetObject(entry.Value, OpenMode.ForRead); if (grp != null && grp.Has(AcadBlockRef)) { return new TransactionResult<(bool, Group)>((true, grp), TransactionAction.Nothing); } }
                    return new TransactionResult<(bool, Group)>((false, null), TransactionAction.Nothing);
                }
                catch (Exception e) { Debug.Print(e.ToString()); return new TransactionResult<(bool, Group)>((false, null), TransactionAction.Nothing); }
            });
        ...
        {
            TransactionResult<(bool Success, BlockReference OutValue)> operationResult = PlugInBlockReference_Helpers.ExecuteWithTransaction(AcadDoc.Database, tr, currTr =>
            {
                try
                {
                    RXClass blockReferenceClass = RXObject.GetClass(typeof(BlockReference));
                    var allEntityIds = BlockRefGroup.GetAllEntityIds();
                    var reportTableHeaderBlockRef = allEntityIds.Cast<ObjectId>()
                        .Where(id => id.ObjectClass.IsDerivedFrom(blockReferenceClass))
                        .Select(id => (BlockReference)currTr.GetObject(id, OpenMode.ForRead))
                        .FirstOrDefault(br => br.AttributeCollection.Cast<ObjectId>().Select(id => (AttributeReference)currTr.GetObject(id, OpenMode.ForRead)).Any(att => att.Tag.Equals("table_header_block_check_string", StringComparison.OrdinalIgnoreCase) && att.TextString.Equals("FoundOK", StringComparison.OrdinalIgnoreCase)));
                    return new TransactionResult<(bool, BlockReference)>((true, reportTableHeaderBlockRef), TransactionAction.Nothing);
                }
                catch (Exception e) { Debug.Print(e.ToString()); return new TransactionResult<(bool, BlockReference)>((false, null), TransactionAction.Nothing); }
            });
            return operationResult.Result.OutValue;
        }
        ...
        public bool Refresh_ReportTable(Transaction tr = null)
        {
            ...
            TransactionResult<bool> operationResult = PlugInBlockReference_Helpers.ExecuteWithTransaction(AcadDoc.Database, tr, currTr =>
            {
                try
                {
                    ...
                    var reportHeaderTableObjectId = blockTableRecord.Cast<ObjectId>().FirstOrDefault(id => id.ObjectClass.IsDerivedFrom(tableClass));
                    if (reportHeaderTableObjectId.IsNull || reportHeaderTableObjectId.IsErased) { return new TransactionResult<bool>(false, TransactionAction.Nothing); }
                    var reportHeaderTable = (Table)currTr.GetObject(reportHeaderTableObjectId, OpenMode.ForRead);
                    if (reportHeaderTable == null) { return new TransactionResult<bool>(false, TransactionAction.Nothing); }

                    var attRefDict_HeaderBlock = PlugInBlockReference_Helpers.Get_AttRefDict(ReportTableHeaderBlockRef, currTr);
                    var excelFilePathType = PlugInBlockReference_Helpers.TryGetValue_AttRefDict(attRefDict_HeaderBlock, "EXCEL_FILEPATH_TYPE")?.TextString?.Trim();
                    var excelFilePath = PlugInBlockReference_Helpers.TryGetValue_AttRefDict(attRefDict_HeaderBlock, "EXCEL_FILEPATH")?.TextString?.Trim();
                    var excelSheetName = PlugInBlockReference_Helpers.TryGetValue_AttRefDict(attRefDict_HeaderBlock, "EXCEL_SHEETNAME")?.TextString?.Trim();
                    var reportTableHandle = PlugInBlockReference_Helpers.TryGetValue_AttRefDict(attRefDict_HeaderBlock, "REPORT_TABLE_HANDLE")?.TextString?.Trim();
                    if (string.IsNullOrEmpty(excelFilePath)) { return new TransactionResult<bool>(false, TransactionAction.Nothing); }

                    var excelFilePathResolved = excelFilePathType?.Equals("ABSOLUTE", StringComparison.OrdinalIgnoreCase) == true
                        ? excelFilePath
                        : System.IO.Path.Combine(System.IO.Path.GetDirectoryName(AcadDoc.Name) ?? string.Empty, excelFilePath);
                    if (!System.IO.File.Exists(excelFilePathResolved)) { Debug.Print($"Excel file not found: {excelFilePathResolved}"); return new TransactionResult<bool>(false, TransactionAction.Nothing); }

                    using var excelWorkBook = new XLWorkbook(excelFilePathResolved);
                    var excelWorkSheet = string.IsNullOrEmpty(excelSheetName) ? excelWorkBook.Worksheet(1) : excelWorkBook.Worksheet(excelSheetName);
                    var excelRange = excelWorkSheet.RangeUsed();
                    if (excelRange == null) { return new TransactionResult<bool>(false, TransactionAction.Nothing); }
                    var excelTable = excelRange.AsTable();
                    var headerRow_HeaderBlock = reportHeaderTable.Rows[0];

                    using var newTable = new Table { TableStyle = reportHeaderTable.TableStyle };
                    newTable.SetSize(1, reportHeaderTable.Columns.Count);
                    newTable.SetRowHeight(headerRow_HeaderBlock.Height);
                    for (int i = 0; i < newTable.Columns.Count; i++) { newTable.Columns[i].Width = reportHeaderTable.Columns[i].Width; newTable.Columns[i].TextHeight = reportHeaderTable.Columns[i].TextHeight; }
                    ...
                    foreach (var tableRow in excelTable.DataRange.Rows())
                    {
                        ...
                        foreach (var headerCell_HeaderBlock in headerRow_HeaderBlock)
                        {
                            ...
                            string columnHeaderName = columnHeaderCell.TextString.Trim();
                            if (string.IsNullOrWhiteSpace(columnHeaderName)) { continue; }
                            var excelCell = tableRow.Field(columnHeaderName);
                            string excelCellValue = excelCell == null ? string.Empty : excelCell.GetFormattedString();
                    ...
                    if (!string.IsNullOrEmpty(reportTableHandle) && long.TryParse(reportTableHandle, System.Globalization.NumberStyles.HexNumber, null, out long handleValue))
                    {
                        try
                        {
                            ObjectId reportTableId = AcadDoc.Database.GetObjectId(false, new Handle(handleValue), 0);
                            if (!reportTableId.IsNull && !reportTableId.IsErased) { reportTable = currTr.GetObject(reportTableId, OpenMode.ForWrite) as Table; }
                        }
                    ...
                    if (reportTable != null)
                    {
                        // HandOverTo transfers the database-resident table content into the replacement table.
                        reportTable.HandOverTo(newTable, false, false);
                        currTr.AddNewlyCreatedDBObject(newTable, false);
                    }
                    else
                    {
                        var blockTable = (BlockTable)currTr.GetObject(AcadDoc.Database.BlockTableId, OpenMode.ForRead);
                        var modelSpace = (BlockTableRecord)currTr.GetObject(blockTable[BlockTableRecord.ModelSpace], OpenMode.ForWrite);
                        ...
                        if (handleAttRef != null) { handleAttRef.UpgradeOpen(); handleAttRef.TextString = newTable.Handle.ToString(); }
                    }

                    if (BlockRefGroup != null && !newTable.IsDisposed && !newTable.IsErased)
                    {
                        var group = (Group)currTr.GetObject(BlockRefGroup.ObjectId, OpenMode.ForWrite);
                        if (!group.Has(newTable)) { group.Append(new ObjectIdCollection(new[] { newTable.ObjectId })); }
                    }
                    return new TransactionResult<bool>(true, TransactionAction.Commit);
                }
                catch (Exception e) { Debug.Print(e.ToString()); return new TransactionResult<bool>(false, TransactionAction.Abort); }
            });
